...
2. **Prototype couche interface** : `ApiGenerator.java` (`doGenerateInheritance`) boucle désormais sur tous les parents et émet une `Generalization` par parent, au lieu de s'arrêter à l'index 0. Sur décision de Juan, pas une consigne de Cédric.
3. **Risque `@objid` levé** : ce checkout SemGen est bien la source de vérité, pas un round-trip depuis un modèle Modelio séparé de SemGen — l'édition manuelle des `.java` est sûre.
4. **Décision côté `Impl`, tranchée par Juan** : entre (a) aplatir les membres du 2ᵉ parent directement sur la classe `Impl` feuille, et (b) générer automatiquement la délégation vers un objet composé (l'équivalent automatisé du contournement actuel) — **(a) retenu**, seule option réglant vraiment les 4 réserves de Cédric (§4), et alignée avec le précédent EMF/Ecore déjà cité comme référence MDE.
5. **Aplatissement implémenté** (source-only, ni compilé ni testé — pas de build dans ce checkout) :
   - Nouvel utilitaire partagé `utils/ModelUtils.java` : `getSecondaryAncestorsToFlatten(mmClass)` remonte toute la chaîne d'ancêtres du 2ᵉ parent (récursif, gère un parent primaire lui-même multi-parenté), dédoublonné contre la chaîne primaire et entre branches secondaires multiples — répond au point de vigilance « aplatir toute la chaîne, pas juste les membres propres ». `getFlattenedOwnedAttributes`/`getFlattenedOwnedEnds` combinent membres propres + membres des ancêtres secondaires, en ignorant (et journalisant) toute collision de nom plutôt que de produire un doublon silencieusement cassé — répond partiellement au point de vigilance sur les collisions (traitement *ignorer + avertir*, pas une vraie résolution : un membre en collision n'est simplement pas émis).
   - **8 points d'appel élargis** vers ces versions aplaties : `MetaclassSmAttributeGenerator.run`/`MetaclassSmDependencyGenerator.run` (couche `mc` — la pièce manquante réelle : enregistre les descripteurs `SmAttribute`/`SmDependency` du 2ᵉ parent directement sur le `SmClass` de la classe feuille, puisque `get<Attr>Att()` n'existe que là où il est enregistré et que le `SmClass` de la feuille n'hérite pas de celui du 2ᵉ parent), `MetaclassLoadGenerator.run` (les deux boucles), `DataGenerator.doGenerateAttributes`/`doGenerateAssocs` (ajoute physiquement les champs de stockage du 2ᵉ parent sur la classe `Data` de la feuille), et `AttributeHelper`/`DependencyHelper`/`CompositionAccessorHelper` dans `impgen` (génèrent les corps de méthode `Impl` réels).
   - `ImplGenerator.doGenerateInheritance` lui-même n'a nécessité **aucune modification** : son `createInterfaceRealization(implClass, emfClass)` existant pointe déjà vers l'interface, qui étend désormais correctement les deux parents — Java ne vérifie pas que l'interface implémentée étend elle-même un ou deux parents.
6. **Ouvert, non résolu** : l'interaction avec la persistance par classe de Modelio (`structural.node`) pour une classe qui serait le 2ᵉ parent d'un cas tout en restant une classe `Semantic` normale ailleurs — en principe sans risque (la classe parente n'est jamais modifiée, seules des copies aplaties sont ajoutées sur la feuille), mais non vérifié structurellement depuis le seul code source ; à confirmer par un test de régénération sur un cas concret parmi les 34, s'il en existe un.
7. **Ouvert, signalé côté SemGen, à vérifier ici** : les générateurs de vérification de liens/dépendances en mode Toutatis (`base/toutatis/expert/*`) ne voient toujours que la chaîne primaire — non touchés, faute de savoir si le métamodèle `reference/design` tourne en mode Toutatis. À vérifier de ce côté-ci si pertinent.
8. **Confirmé, `FlowUsage` (cas #22, les 3 parents)** : primaire `ActionUsage`, deux parents secondaires `ConnectorAsUsage` et `Flow` — seul cas des 34 avec deux axes secondaires au lieu d'un, bon cas réel pour tester le code d'aplatissement générique à N parents.
9. **Build réussi** : le patch complet (correctif interface + aplatissement mc/data/impgen) a passé un aller-retour Modelio proprement (les 9 fichiers touchés ont gardé le correctif), puis compilé sans erreur — `SemGen_4.0.00.jmdac` construit via Maven/JDK21 contre `repository.modelio.org`.
10. **Prochaine étape, cadrée par Antonin** : installer ce module dans une instance Modelio et régénérer — **pas dans le fragment SysML2 partagé de `modelio.all`**, mais dans un **projet local séparé**, pour isoler le test d'un module SemGen encore non validé avant de risquer le fragment partagé. Retour attendu une fois un résultat réel observé. ScriptServer indisponible de ce côté-ci pour l'instant (vérifié à nouveau, toujours hors ligne) — pas encore possible de croiser depuis le modèle en attendant.
