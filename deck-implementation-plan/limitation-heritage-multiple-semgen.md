...
> **Risque anticipé** : corriger `ApiGenerator` seul va probablement faire passer le bug de « le 2ᵉ parent disparaît silencieusement mais ça compile » à « `AttributeDefinitionApi` étend correctement `Definition`+`DataType`, mais `AttributeDefinitionImpl` ne compile plus » sur les 34 classes — `ImplGenerator` n'hérite toujours que d'un seul parent concret alors que l'interface qu'il doit satisfaire s'est élargie. La régénération de test prévue vise à confirmer si ce risque se matérialise.
> **Décision côté `Impl`, tranchée par Juan** : entre (a) aplatir les membres du 2ᵉ parent directement sur la classe `Impl` feuille, et (b) générer automatiquement la délégation vers un objet composé (l'équivalent automatisé du contournement actuel) — **(a) retenu**. C'est la seule des deux options qui règle vraiment les 4 réserves de Cédric (§4) : un seul objet réel, pas de surcoût mémoire/perf, pas de cohérence à garantir entre deux objets, pas de façade à construire pour la masquer. Elle rejoint aussi le précédent EMF/Ecore (flattening) déjà cité comme référence MDE pour ce problème précis. Points de vigilance signalés pour l'implémentation : aplatir toute la chaîne d'ancêtres transitive du 2ᵉ parent (pas seulement ses membres propres — une version aplatie construite à la main plus tôt dans ce projet montrait des classes à 41 membres une fois toute la chaîne incluse), vérifier les collisions de nom entre les deux chaînes d'ancêtres, et vérifier l'effet sur la persistance par classe de Modelio (`structural.node`) si une classe joue le rôle de second parent dans un cas tout en restant elle-même une classe `Semantic` normale ailleurs. Pas encore implémenté ni testé au moment de cette mise à jour.
