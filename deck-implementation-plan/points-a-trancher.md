...
**Réserve de Cédric Marin sur ce design** : multiplication des éléments de modèle, coût mémoire/perf, absence de contrôles de cohérence entre les deux objets, besoin d'une façade — le même mur que l'implémentation UML2 de Modelio il y a 15 ans. Traité en détail, avec exemples chiffrés et recommandation en 3 temps, dans `limitation-heritage-multiple-semgen.md` et son support de présentation (`slides-heritage-multiple/`) — document séparé destiné à cette discussion précise, ce point 5 restant le journal technique des essais/décisions. **Suite donnée** : présenté à Cédric, qui a transmis le code source de SemGen. Correctif choisi et implémenté : interface `mm.api` à héritage multiple réel (`ApiGenerator`) + aplatissement des membres du 2ᵉ parent sur la classe `Impl` feuille (8 points d'appel élargis dans les générateurs `mc`/`load`/`data`/`impl`) — plutôt qu'automatiser la délégation existante, seule option réglant vraiment les 4 réserves de Cédric. Build réussi (`SemGen_4.0.00.jmdac`) ; prochaine étape : régénération réelle des composants `KerML`/`SysML` pour vérifier que ça compile, notamment sur `FlowUsage` (cas #22, le seul à 2 parents secondaires). Deux points restent ouverts : l'interaction avec la persistance par classe (`structural.node`) et le mode Toutatis (générateurs de vérification `base/toutatis/expert/*`, non couverts par le correctif). Suivi détaillé, vivant, dans `limitation-heritage-multiple-semgen.md`.
