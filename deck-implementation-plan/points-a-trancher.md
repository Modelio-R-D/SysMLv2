...
**Réserve de Cédric Marin sur ce design** : multiplication des éléments de modèle, coût mémoire/perf, absence de contrôles de cohérence entre les deux objets, besoin d'une façade — le même mur que l'implémentation UML2 de Modelio il y a 15 ans. Traité en détail, avec exemples chiffrés et recommandation en 3 temps, dans `limitation-heritage-multiple-semgen.md` et son support de présentation (`slides-heritage-multiple/`) — document séparé destiné à cette discussion précise, ce point 5 restant le journal technique des essais/décisions. **Suite donnée** : présenté à Cédric, qui a transmis le code source de SemGen. Sur décision de Juan (pas une consigne de Cédric), un prototype de correctif existe pour la couche interface (`ApiGenerator.java`), pas encore testé — les 4 autres générateurs ciblent une vraie classe Java concrète et ne peuvent pas être corrigés de la même façon (limite Java réelle, pas un oubli), ce qui laisse ouvert un risque anticipé mais non confirmé : `ImplGenerator` pourrait ne plus compiler sur les 34 classes une fois l'interface élargie. Décision prise pour ce cas : aplatir les membres du 2ᵉ parent sur la classe `Impl` (plutôt qu'automatiser la délégation existante), seule option réglant vraiment les 4 réserves de Cédric. Pas encore implémenté. Détail complet et suite à venir dans `limitation-heritage-multiple-semgen.md`.
